        ...
        <div class="chart-container">
          <div class="bar-chart">
            <div v-for="(month, index) in monthlyData" :key="index" class="bar-wrapper">
              <div class="bar-value">{{ formatCurrencyShort(month.revenue) }}</div>
...
export default {
  ...
  setup() {
    ...
    const formatCurrency = (num) => {
      ...
      return Number(num).toLocaleString(currency === 'JPY' ? 'ja-JP' : 'en-US', {
        style: 'currency',
        currency,
    ...
    const formatCurrencyShort = (num) => {
      const currency = currentCurrency.value
      if (num >= 1_000_000) {
        return (currency === 'JPY' ? '¥' : '$') + (num / 1_000_000).toFixed(1) + 'M'
      }
      if (num >= 1_000) {
        return (currency === 'JPY' ? '¥' : '$') + (num / 1_000).toFixed(0) + 'K'
      }
    ...
    const formatMonth = (monthStr) => {
      ...
      return t(`months.${keys[parseInt(month) - 1]}`) + ' ' + year
    }
    ...
    const getBarHeight = (revenue) => {
      ...
      return (revenue / maxRevenue.value) * 160
    }
    ...
    const getChangeValue = (current, previous) => {
      ...
      return change > 0 ? '+' + formatted : change < 0 ? '-' + formatted : formatCurrency(0)
    }
    ...
    return {
      ...
      formatCurrency, formatCurrencyShort, formatMonth,
      getBarHeight, getFulfillmentClass,
